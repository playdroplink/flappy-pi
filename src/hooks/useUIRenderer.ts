...
export const useUIRenderer = () => {
  const renderTapToStart = useCallback((
    ...
    const instructionText = isMobile ? 'Touch to fly' : 'Touch the screen or press space to continue flying';
    ctx.fillText(instructionText, canvas.width / 2, canvas.height / 2 + 40);
    
    // Revive indicator
    ctx.font = `${smallTextSize}px Arial`;
    ctx.fillStyle = '#90EE90';
    ctx.fillText('🚀 Bird Revived - Safe Position', canvas.width / 2, canvas.height / 2 + 70);
    ...
    ctx.fillText('Get Ready to Continue!', canvas.width / 2, canvas.height / 2 + 100);
    
    // Draw a hint arrow to indicate tap action
    ctx.save();
    const arrowY = canvas.height / 2 + 140;
    ctx.translate(canvas.width / 2, arrowY);
    ctx.fillStyle = `rgba(144, 238, 144, ${pulseAlpha})`;
